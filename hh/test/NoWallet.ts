...
// import { CONTRACT_NAME, DEPOLYER_CONTRACT_NAME, MIN_RGF, RGF, RGFM, RGF_MANUAL_CONTRACT_NAME } from "../src/constants";
...
//     await wallet.init(cert, RGFProvider.address);
//     await wallet.payment({ value: ethers.utils.parseEther("10.") });
...
//     let proof = passwordsAndAddressAndCertAndNonceToProof(PASSWORD, wallet.address, cert, 0, nonceSize);
...
// //   it("Pending attack counter", async function () {
// //     const [owner, addr1, addr2, addr3] = await ethers.getSigners();
...
// //     let proof = passwordsAndAddressAndCertAndNonceToProof(PASSWORD, wallet.address, cert, nonce, nonceSize);
// //     let {txCert} = signTransactionAndProof({ to, data, value }, proof+'00');

// //     const gweiValue = MIN_RGF;
// //     const RGF = BigNumber.from('7037175940');
// //     try {
// //       for (let i = 0; i < 300; i++) {
// //         console.log({i});
// //         await wallet.call(addr1.address, value, [], txCert, { value: RGF });
// //       }  
// //     } catch(e: any) {

// //     }

// //     let pendingAttackCounter = await wallet.pendingAttackCounter();
// //     console.log({ pendingAttackCounter });
// //     await wallet.expose(proof, 0);
// //     pendingAttackCounter = await wallet.pendingAttackCounter();
// //     console.log({ pendingAttackCounter });
...
// //       proof = passwordsAndAddressAndCertAndNonceToProof(PASSWORD, wallet.address, cert, nonce, nonceSize);
...
//   //   const RGFProvider = await ethers.getContractFactory(RGF_MANUAL_CONTRACT_NAME);
//   //   // const rgfIface = RGFProvider.interface;
//   //   let rgfProvider = RGFProvider.attach(await wallet.rgfProvider());
//   //   // console.log({ rgfProvider });
//   //   // console.log(await rgfProvider.get(data.length-2));
...
//   //   let maxFeePerGas = ethers.BigNumber.from(40_000_000_000) // fallback to 40 gwei
//   //   let maxPriorityFeePerGas = ethers.BigNumber.from(40_000_000_000) // fallback to 40 gwei
//   //   let gasPrice = ethers.BigNumber.from(1_528_965_689) // fallback to 40 gwei

//   //   const gweiValue = MIN_RGF;
//   //   // const fees = gweiValue.mul(BigNumber.from(RGFM));
//   //   const fees = (await rgfProvider.getManual((data.length-2)/2, gasPrice));
//   //   console.log((data.length-2)/2)
//   //   // console.log(fees);
//   //   console.log(fees);
//   //   await wallet.call(wallet.address, value, data, txCert, { value: fees, maxFeePerGas });
//   //   await wallet.expose(proof, 0);
...
//   //   await wallet.call(to, value, data, txCertNew, { value: gweiValue.mul(BigNumber.from(RGFM)), maxFeePerGas });
//   //   await wallet.expose(proof, 0);
//   //   console.log({ balance: ethers.utils.formatEther(await addr1.getBalance()) });
//   // });
...
// //     const [owner, addr1, addr2, addr3] = await ethers.getSigners();
// //     let privateKey = "0xdbda1821b80551c9d65939329250298aa3472ba22feea921c0cf5d620ea67b97";
// //     let addr = new ethers.Wallet(privateKey, owner.provider);
// //     console.log({ balance: ethers.utils.formatEther(await addr.getBalance()) });

// //     const TwoFactorWallet = await ethers.getContractFactory(CONTRACT_NAME);
// //     console.log({addr1})
// //     const wallet = await TwoFactorWallet.connect(addr).deploy();
...
//   //   // console.log({addr1})
//   //   const deployer = await Depolyer.deploy();
//   //   // console.log({ deployer })
//   //   console.log({ balance: ethers.utils.formatEther(await owner.getBalance()) });

//   //   let walletAddress = await deployer.callStatic.createAccount();
