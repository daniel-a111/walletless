...
const AppHeader = () => {
    ...
    const [chainCurrency, setChainCurrency] = useState<string|undefined>();
    ...
    return <>
        ...
        {pendingCount > 0 && <div className="btn-pending">{pendingCount} pendings...</div>}
            <h1>timeLOCK
            <span className="tag-version">ALPHA</span>
        </h1>
        <h2>Prevent wallet theft using two-factor transaction protection</h2>

        {!wallet &&
            <><button className="btn-connect-wallet" onClick={onClickConnectMetaMask}>
            ...
            <div>
                {wallet &&
                    <>
                        ...
                        <div className="currency">{chainCurrency}</div>
                    </>
                }
                {!wallet &&
                    <button onClick={onClickConnectMetaMask}>Connect MetaMask</button>            
                }
            </div>
        }
    </>;
